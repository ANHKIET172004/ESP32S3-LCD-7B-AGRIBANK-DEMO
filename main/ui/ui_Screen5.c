...
static int device_index[MAX_DEVICES];
...
extern device_info_t device_list[];
extern int device_count;
extern esp_err_t load_device_list_from_nvs(void);


char selected_device_id[18]={0};
...
esp_err_t read_selected_device_id1(char *out_id, size_t out_len)
{
    ...
    if (err == ESP_ERR_NVS_NOT_FOUND) {
        ESP_LOGW("READ_DEVICE_ID", "Key \"device_id\" không tồn tại");
    }
    else if (err == ESP_ERR_NVS_INVALID_LENGTH) {
        ESP_LOGE("READ_DEVICE_ID", "Chuỗi trong NVS dài hơn buffer (%d > %d)", required_size, out_len);
    }
    else if (err == ESP_OK) {
...
void ui_Screen5_screen_init(void)
{
    ...
    lv_img_set_src(ui_Image22, &ui_img_turn_back2_png);
    lv_obj_set_width(ui_Image22, LV_SIZE_CONTENT);   /// 70
    lv_obj_set_height(ui_Image22, LV_SIZE_CONTENT);    /// 70
    lv_obj_set_x(ui_Image22, -447);
    lv_obj_set_y(ui_Image22, 247);
    ...
    lv_obj_add_event_cb(ui_Image22, ui_event_Image44, LV_EVENT_ALL, NULL);

    ui_DeviceList_Container = lv_obj_create(ui_Screen5);
    lv_obj_set_size(ui_DeviceList_Container, 240, 280);
    lv_obj_center(ui_DeviceList_Container);

    // Gọi hàm hiển thị danh sách thiết bị
    load_device_list_from_nvs();
    ui_show_device_list(ui_DeviceList_Container);
