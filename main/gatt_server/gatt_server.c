 ...
 void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_WRITE_EVT: {
        ...
        if (!param->write.is_prep){
            ESP_LOGI(GATTS_TAG, "Received message length: %d", param->write.len);
            
            // In tin nhắn nhận được
            char received_msg[256] = {0};
            memcpy(received_msg, param->write.value, param->write.len);
            ESP_LOGI(GATTS_TAG, "***** Received message: %s *****", received_msg);
            
            // Gửi response
            char response[300];
            snprintf(response, sizeof(response), "Server received: %s", received_msg);
    ...
    case ESP_GATTS_CREATE_EVT:
        ...
        gl_profile_tab[PROFILE_A_APP_ID].char_uuid.uuid.uuid16 = 0xFF01;
...
void ble_server_task(void *arg)
{
    ...
    // Đăng ký GATT server callback
    //esp_ble_gatts_register_callback(gatts_event_handler);
    //esp_ble_gatts_app_register(PROFILE_A_APP_ID);

    // Task BLE chạy vòng lặp chờ sự kiện
    while (1) {
